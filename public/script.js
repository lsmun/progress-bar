...
class Matrix {
        ...
        for(var i = 0; i < rows; i++){
            for(var k = 0; k < columns; k++) {
                if(k < columnGoToArray[i]) {
                    newMatrix[i][k] = true;
                  //  document.getElementById(i+'-'+k).checked = true;
                } else {
                    newMatrix[i][k] = false;
                 //   document.getElementById(i+'-'+k).checked = false;
                }
            }
           
            //newMatrix[i] = myMatrix;
        }
        
        for(var i = 0; i < rows; i++){
            for(var k = 0; k < columns; k++) {
               setTimeout(function(value,element) {
                   console.log("-- "  + value);
                  element.checked = value;
                }, i * k * 100, newMatrix[i][k], document.getElementById(i+'-'+k));
            }
        }
 ...
 /*   getInfo(Latitude, Longitude){
        ...
        var theURL = 'http://samples.openweathermap.org/data/2.5/forecast?lat='+newLat+'&lon='+newLong+'&appid=b1b15e88fa797225412429c1c50c122a1';
...
var tempArray = sampleMatrix.getInfo(data,sampleMatrix.matrix.length);



sampleMatrix.timedLoop(tempArray);
